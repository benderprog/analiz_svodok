...
Документ описывает работу в UI без погружения в код.

## Роли и вход
- Роли **admin** и **operator** сейчас одинаковы по правам.
- Вход выполняется через страницу `/login` с учётной записью из `.env` (`APP_ADMIN_LOGIN` / `APP_ADMIN_PASSWORD`).
...
- Если время найдено, сравнение идёт с точностью до **±30 минут** (по умолчанию).
- Статусы:
  - `+` — время совпало точно.
...
**Offenders**
- Сравнение по множеству нарушителей (ФИО + дата рождения).
- Статусы:
  - `+` — полный набор совпал.
...
- **Не извлеклись нарушители**
  ...
  - Дата рождения поддерживается как `дд.мм.гггг` или `1991 г.р.`.
...
В релизе есть папка `fixtures/` с текстовыми примерами. Для UI нужен DOCX — его можно сгенерировать:
```bash
